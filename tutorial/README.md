...
`tutorial/` 配下のファイルはそのまま使います（コピー不要）。

```bash
cd /path/to/insight-blueprint
ls tutorial/sample_data/sales.csv   # データが存在することを確認
ls tutorial/notebooks/              # 01_explore.py, 02_verify.py
```
...
```
tutorial/sample_data/sales.csv をカタログに登録して
```
...
```bash
marimo edit tutorial/notebooks/01_explore.py
```
...
```bash
marimo edit tutorial/notebooks/02_verify.py
```
...
`tutorial/sample_data/sales.csv` は架空のコーヒーチェーン（3店舗、6ヶ月間）の売上データです。
