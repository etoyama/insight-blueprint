...
def infer_user_stories(design: dict, research: list) -> list[str]:
    ...
    if "authentication" in design_content.lower():
        stories.append(
            "As a user, I want to securely log in, so that I can access protected features"
        )

    if "api" in design_content.lower():
        stories.append(
            "As a developer, I want to integrate via API, so that I can automate workflows"
        )
...
def main():
    ...
    extraction = {
        ...
        "inferred_user_stories": infer_user_stories(
            extract_design_decisions(design_file), extract_research(research_dir)
        ),
    }
