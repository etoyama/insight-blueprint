...
- **Lessons 記録**: `{RUN_DIR}/lessons.md` に知見を記録（共有 rules への昇格は人間が朝レビュー時に実施）
...
| **.claude/rules/marimo-notebooks.md** | marimo 固有のルール（_prefix, plt.gcf() 等）。参照のみ（夜間バッチは直接更新しない） | 既存ファイル |
...
flowchart TD
    ...
    Transition --> Reset["MCP: update next_action → {}"]
...
marimo 固有のエラーを修正した場合、`{RUN_DIR}/lessons.md` に以下の形式で記録（共有 rules への昇格は人間が朝レビュー時に実施）:
